...
destroy_room(Nick, Socket, Content) ->
  ...
  case Response of
    ok ->
      ...
      ok;
    {error, MESSAGE} ->
      gen_tcp:send(Socket, "DESTROY_ROOM:ERROR:" ++ MESSAGE ++ "\n"),
      ok
  end,
  handle_command(Nick, Socket).
...
leave_room(Nick, Socket) ->
  Response = gen_server:call(?CONTROLLER, {join_room, Nick, "Home"}),
  case Response of
    ok ->
      gen_tcp:send(Socket, "LEAVE_ROOM:OK:Room left\n"),
      ok;
    {error, MESSAGE} ->
